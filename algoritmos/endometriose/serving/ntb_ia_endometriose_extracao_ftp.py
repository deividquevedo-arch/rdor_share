...
def get_data(unidades):
  df = spark.sql(f"""
      select
        ...
        numCrm as numCrmSolicitante,
        ufCrm as ufCrmSolicitante,
        nomeMedico as nomeMedicoSolicitante,
        ...
        codigo as codigoTuss,
        nomeCodigo as descricaoTuss,
        descricaoProcedimento,
        laudoOriginal as laudoExame,
        classificacao
...
def move_file(source, target, extension):
    for file in dbutils.fs.ls(source):
...
def export_to(df, unidade, extension):
    file_name = f"{id_projeto.capitalize()}-{unidade}-{data_execucao_modelo}{extension}"
    local_file = f"{current_folder}{file_name}"
    remote_file = f"{remote_path}{file_name}"
    ...
    if extension == ".csv":
        df.repartition(1).write.mode("overwrite").option("quote", '"').csv(temp_remote_path, header = True)
 
    move_file(temp_remote_path, remote_file, extension)
...
def extract_data(unidade, ids, formato):
    print("-" * 120)
    ...
    _info = export_to(_df_export, unidade, formato)
...
df = spark.read.option("multiline","true").json(f"{root_remote_config_path}unidades_ftp.json").where("enabled == true")
df.display()
...
for row in df.collect():
    ...
    info = extract_data(row.unidade, row.unidades, row.formato)
    files.append(info)
...
#password = "RedeDor@2023"  # TODO: Ler senha do Key-vault
