...
**O que:** criar **3 libs independentes** em **multi-repo**, extraindo o melhor codigo existente, **sem dependencia de embeddings ou GPU**.

**3 libs (multi-repo, sem imports cruzados):**

| Lib | Escopo | Modulos |
|-----|--------|---------|
| **`nlp_engine`** | Processamento NLP puro | `text_pipeline.py`, `engine.py`, `config_loader.py` (com merge `shared/organs.yaml`), `scoring.py` |
| **`data_manage`** | Gestao de dados (I/O Delta, contratos) | `loader.py`, `saver.py`, `contracts.py`, `config_loader.py` |
| **`monitoring`** | Observabilidade (metricas, logs) | `metrics.py`, `quality_guard.py`, `config_loader.py` |
...
**Repo de especialidades (`plataforma-nlp/`):**
```
plataforma-nlp/
  shared/
    organs.yaml                     — universo de orgaos (runtime, ref. pelo nlp_engine)
  especialidades/
    _template/                      — molde para nova especialidade
      config.yaml
      ntb_motor.py
    hepatologia/
      config.yaml                   — secoes: nlp, data, monitoring
      ntb_motor.py                  — notebook orquestrador (Composition Root)
    biliar/
    colon/
    ...
  tests/integration/
  deploy/
    integration/                    — build-time YAMLs (MLOps)
  requirements.txt                  — versoes das 3 libs pinadas
```
...
### A4 — Externalizar configuracao clinica em YAML (duas camadas)

**O que:** mover CONFIG de dentro dos notebooks para YAML versionado no Git.

**Config em duas camadas:**

- **Camada compartilhada** (`shared/organs.yaml`): universo de orgaos para desambiguacao. Vive no repo de especialidades. Carregado em runtime pelo `nlp_engine` via path no YAML da especialidade. Fonte unica -- atualizar 1 arquivo beneficia todas as especialidades.
- **Camada de especialidade** (`especialidades/{nome}/config.yaml`): 1 arquivo por especialidade com secoes por lib:
  - `nlp`: target_organs, findings, thresholds, negation_window, shared_organs_path
  - `data`: input_table, output_table, catalog
  - `monitoring`: metrics_table, alert_thresholds, baseline
- **YAML build-time** (`deploy/integration/{nome}.yaml`): tabelas, catalogo, lib_version. Responsabilidade MLOps. Alimenta esteira de code generation.

**Notebook orquestrador (Composition Root):** le o YAML da especialidade, extrai secoes e injeta dict em cada lib. Nenhuma lib le o arquivo diretamente.

**Resolve lacunas:** L8, L20, L17 (versionamento de regra clinica via Git tag)

### A5 — Organizar codigo (notebooks -> 3 libs + notebooks orquestradores)
...
**Proposta:**
- Logica de negocio em 3 libs independentes (multi-repo, testavel com pytest, sem Spark)
- Notebooks Databricks ficam com ~50 linhas: widgets -> carregar config -> chamar 3 libs -> gravar Delta
- Serving layer existente (extracao Excel/SharePoint/API) continua inalterado
- CI: adicionar lint + testes unitarios de cada lib como gate antes do sync

**Decisao tomada:** multi-repo (1 repo por lib: `nlp_engine`, `data_manage`, `monitoring`) com wheel no feed interno
