...
```
Laudo (raw) --> [nlp_engine: TextPipeline + RuleBasedEngine] --> [monitoring: metricas] --> [data_manage: save Delta] --> Excel/SharePoint
```

**Sem encoder, sem GPU, sem treinamento.** O processamento fica na lib `nlp_engine` (TextPipeline + RuleBasedEngine); `monitoring` expoe metricas antes da persistencia; `data_manage` centraliza o save em Delta e o encaminhamento ao canal operacional. Na Fase 2, embeddings existentes (MiniLM) entram como componente opcional de `semantic_expand` em `nlp_engine`.
...
| `nlp_engine` | TextPipeline + RuleBasedEngine (spaCy, regex, YAML) | **1** | Lib do motor de texto e regras; consumida pelos notebooks |
| `data_manage` | Delta Lake (Unity Catalog), contratos I/O | **1** | Lib de persistencia e save Delta versionado |
| `monitoring` | Metricas de pipeline e qualidade | **1** | Lib de observabilidade entre processamento e persistencia |
...
| 2 | **Repositorio da lib NLP** | **Resolvido:** multi-repo (1 repo por lib: `nlp_engine`, `data_manage`, `monitoring`) com wheel no feed interno | Aplicado: 3 wheels no feed | **1** | Define como notebooks consomem as libs |
| 3 | **Runtime e paths no YAML da especialidade** | Contrato compartilhado entre libs: versao de runtime, paths de artefatos e escopo por especialidade no YAML | Alinhar EngML + DS (contrato minimo) | **1** | Evita deriva entre `nlp_engine`, `data_manage` e `monitoring` |
| 4 | **Cluster GPU para treinamento** | Tipo de VM, policy, scheduling | Single node A10 ou V100; job cluster on-demand | **3** (nao antes) | CPT e fine-tuning exigem GPU; Fases 1-2 nao |
| 5 | **CI/CD gates** | (A) Manter sync only, (B) Adicionar lint+testes, (C) Gate completo com testes NLP | B: lint + testes unitarios da lib como pre-req | **2** | Reduz risco de regressao sem travar velocity |
| 6 | **Padrao de imagem/init script** | (A) Init script com pip, (B) Container image customizada, (C) Cluster policy com libs | B ou C: imagem com deps fixadas | **2** | Elimina pip ad hoc e deriva |
| 7 | **MLflow -- padrao de uso** | Experiment naming, model registry, retention | Conforme secao 4 acima | **3** | Garante rastreabilidade de artefatos |
| 8 | **Unity Catalog -- naming** | Padrao de catalog/schema/table para motor unificado | `{env}.ia.tb_diamond_mod_{specialty}_{stage}` | **2** | Consistencia entre especialidades |
| 9 | **Canal de saida** | Excel/SharePoint permanece; Delta como fonte intermediaria | Confirmar com operacao | **1** | Nao bloqueia motor; serving layer inalterado |
...
| 2 | Criar 3 repos (`nlp_engine`, `data_manage`, `monitoring`) + configurar CI minimo (decisao #2) | EngML + DS | Semana 1 |
| 3 | Definir padrao CI/CD minimo (decisao #5) | EngML + DS | Semana 1 |
| 4 | Catalogar dados rotulados e nao-rotulados por especialidade | DS | Semana 1 |
| 5 | Confirmar canal de saida oficial com operacao (decisao #9) | DS + Operacao | Semana 1 |
| 6 | Definir metricas de sucesso com equipe clinica | DS + Clinico | Semana 1-2 |
| 7 | Alinhar padrao Unity Catalog naming (decisao #8) | EngML + DS | Semana 2 |

**Nota:** provisionar cluster GPU (#4), padronizar MLflow (#7) e definir padrao de imagem (#6) sao acoes da **Fase 2-3**, nao bloqueiam Fase 1.
