...
**Ambito:** 9 especialidades ativas (hepatologia, colon, DII, birads, pulmao, rim*, biliar, neuroimunologia, reumatologia) + template `modelo`. Endometriose descontinuado. *Rim: caso especial (business rules, sem NLP textual), evolucao a avaliar.
**Contexto:** entregavel da **Sprint 1** -- estudo, diagnostico tecnico e proposta de evolucao arquitetural.
**Nota de revisao (v0.4):** arquitetura atualizada para 3 libs independentes (`nlp_engine`, `data_manage`, `monitoring`) em multi-repo; config em duas camadas (shared + specialty) com YAML runtime e build-time; notebook como Composition Root; design patterns documentados; decisoes pendentes atualizadas.
...
- a **direcao recomendada**: **3 libs independentes** (`nlp_engine`, `data_manage`, `monitoring`) em multi-repo, consumidas por um notebook orquestrador (Composition Root) com **config YAML centralizado por especialidade**;
...
| **Direcao** | **Evolucao controlada** (nao substituicao): **3 libs independentes** (`nlp_engine`, `data_manage`, `monitoring`) em multi-repo + **config YAML centralizado por especialidade** (Composition Root); notebooks reduzidos a **orquestradores** (~50 linhas); **contrato de entrada/saida** versionado. Pipelines atuais coexistem com o motor novo ate validacao de paridade. |
...
A arquitetura e composta por **3 libs independentes** em **multi-repo**, sem imports cruzados, comunicando por dados (dicts/DataFrames):

| Lib | Responsabilidade | Repo proprio |
|-----|-----------------|-------------|
| **`nlp_engine`** | Processamento NLP: TextPipeline (limpeza, negacao, segmentacao) + ClinicalNlpEngine (regras, embeddings, encoder futuro) + config loader com merge de `shared/organs.yaml` | Sim |
| **`data_manage`** | Gestao de dados: leitura/escrita Delta, validacao de contratos I/O, config loader proprio | Sim |
| **`monitoring`** | Observabilidade: metricas (precision, recall, F1), logging estruturado, baselines, config loader proprio | Sim |

**Design patterns aplicados:**
- **Composition Root:** notebook orquestrador le YAML da especialidade e injeta secao relevante (dict) em cada lib. Nenhuma lib sabe de onde veio a config.
- **Hexagonal Architecture (Ports & Adapters):** libs sao nucleo agnostico; Databricks, Delta, Excel sao adaptadores externos.
- **Template Method:** notebook e o esqueleto fixo (carregar config -> carregar dados -> processar NLP -> emitir metricas -> salvar). YAML e a variacao.

A arquitetura evolui em **4 fases**. As **Fases 1-2** usam apenas engines rule-based + embeddings existentes (**CPU**, sem GPU). O **Shared Encoder + Multi-Task Learning** e o **objetivo de evolucao a partir da Fase 3**, com gate de validacao por fase.

**Configuracao em duas camadas:**
- **Camada compartilhada:** `shared/organs.yaml` no repo do `nlp_engine`, carregado em runtime via path configuravel no YAML da especialidade. Contem universo de orgaos para desambiguacao (fonte unica, todas as especialidades se beneficiam).
- **Camada de especialidade:** 1 YAML por especialidade com secoes por lib (`nlp`, `data`, `monitoring`). Vive no mesmo diretorio do notebook da especialidade.
- **YAML build-time:** parametros de integracao (tabelas, catalogo, lib_version) em `deploy/integration/`. Responsabilidade do MLOps. Alimenta esteira de code generation.

**Repo de especialidades** (`plataforma-nlp/`): contem `shared/`, `especialidades/{nome}/` (YAML + notebook), `tests/integration/`, `deploy/`. Nova especialidade = copiar template, ajustar YAML.

### 8.2 Frame operacional (notebook orquestrador)

O notebook de cada especialidade atua como **Composition Root**: le o YAML da especialidade, extrai secoes e injeta em cada lib. O fluxo e fixo (Template Method); o conteudo varia por YAML:

```
config = yaml.safe_load("hepatologia.yaml")
df = data_manage.load(config["data"])
result = nlp_engine.process(df, config["nlp"])
monitoring.emit(result, config["monitoring"])
data_manage.save(result, config["data"])
```

Orquestracao permanece como **SQL + Jobs Databricks** (pipeline linear agendada). Nao ha reescrita de orquestracao.

**Separacao de responsabilidades:** ingestao SQL, **Excel / SharePoint / OneDrive** (canal oficial atual), FTP, API e dashboards sao **adaptadores** em volta do nucleo NLP e permanecem inalterados; o motor alimenta Delta e o serving layer existente continua consumindo.
...
| ID | Tema | Status | Decisao / Opcoes | Fase |
|----|------|--------|------------------|------|
| A | Hospedagem das bibliotecas | **Resolvido** | Multi-repo: 1 repo por lib (`nlp_engine`, `data_manage`, `monitoring`) com wheel no feed interno | 1 |
| B | Onde vivem as regras | **Resolvido** | YAML em Git, duas camadas (shared + specialty). Ref. `anexo04`. Possibilidade futura de Delta. | 1 |
| C | Produto piloto | **Resolvido** | Hepatologia (prioridade definida com lideranca) | 1 |
| D | Rigidez do CI | Pendente | Somente sync **vs** incluir testes da lib como pre-requisito | 2 |
| E | Formato padrao de artefato | Pendente | Notebooks `.py` exportados **vs** `.ipynb` para novos projetos | 1 |
| F | Canal de saida | **Resolvido** | Excel/SharePoint permanece; Delta como fonte intermediaria | 1 |
| G | Clusters GPU | Pendente | Definicao de cluster GPU a alinhar com EngML/MLOps | 3 |
| H | Runtime Python | **Resolvido** | Unificado para o motor; migracao em ondas sem quebrar PRD | 1 |
