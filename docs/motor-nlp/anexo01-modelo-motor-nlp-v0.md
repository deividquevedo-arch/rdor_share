...
Um **ecossistema NLP modular** -- tres bibliotecas Python independentes (`nlp_engine`, `data_manage`, `monitoring`) em **multi-repo**, parametrizavel e versionado -- que processa laudos clinicos de qualquer especialidade medica. Hoje existem 9 algoritmos independentes fazendo isso com logica duplicada; a proposta consolida o melhor de cada um nessas libs reutilizaveis.

**Bibliotecas e responsabilidades:**

- **`nlp_engine`** -- **TextPipeline** (preparacao de texto: limpeza HTML/RTF/rodape, normalizacao, segmentacao por secao/orgao, negacao com 23 expressoes e janela de 7 tokens) + **ClinicalNlpEngine** (interface unica de processamento clinico; implementacao interna varia por especialidade) + **config loader** (merge de configuracao compartilhada com `shared/organs.yaml` e overrides por orgao/especialidade).
- **`data_manage`** -- I/O **Delta**, **contracts** de dados, **config loader** para parametros de leitura/escrita e validacao de schema.
- **`monitoring`** -- **metrics**, **quality guard**, **logging**, **config loader** para limites e politicas de observabilidade.

**Composition Root (notebook orquestrador):** um notebook atua como ponto unico que le um **YAML centralizado** e **injeta dicionarios** (secoes de configuracao) em cada biblioteca via API. Cada lib e uma **caixa preta**: recebe `dict` por chamada, **nao** conhece arquivo YAML nem o caminho no disco.
...
Laudo clinico (texto bruto)
        ...
        v
+----------------------------------+
|    Notebook orquestrador         |
|    (Composition Root)            |
|    Le config.yaml, injeta        |
|    secoes em cada lib            |
+----------------------------------+
        |
   +----+----+----+
...
|engine  | |manage  | |ing     |
|        | |        | |        |
|TextPipe| |Load    | |Metrics |
|Engine  | |Save    | |Quality |
|Scoring | |Contract| |Logging |
+--------+ +--------+ +--------+
        |
        v
...
|---------|-------|---------|
| Logica NLP (regras, negacao, embeddings) | Reorganiza | Sai de notebooks individuais, entra nas tres bibliotecas centralizadas (`nlp_engine`, `data_manage`, `monitoring`) |
...
**Proposta:** tres bibliotecas (`nlp_engine`, `data_manage`, `monitoring`) mais um notebook como **Composition Root** (YAML centralizado, injecao de configuracao), parametrizavel por especialidade, evoluindo em 4 fases. Comeca simples (regras, CPU), valida, e so entao evolui para tecnicas avancadas (encoder, GPU).
