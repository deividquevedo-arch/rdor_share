...
```
                    3 REPOS DE LIBS (independentes, multi-repo)
                    ==========================================

nlp_engine/                          <-- repo 1
  text_pipeline.py
  engine.py
  config_loader.py                   <-- merge automatico: shared/organs + specialty
  scoring.py
  semantic_expand.py                 <-- Fase 2, opcional
  tests/
...
monitoring/                          <-- repo 3
  ...
  tests/
                    ...
                    REPO DE ESPECIALIDADES (1)
                    ==========================

plataforma-nlp/
  shared/
    organs.yaml                      <-- universo de orgaos (runtime)
  especialidades/
    _template/
      config.yaml                    <-- molde para nova especialidade
      ntb_motor.py
    hepatologia/
  ...
  requirements.txt                   <-- versoes das 3 libs pinadas
                    ...
                    NOTEBOOKS DATABRICKS (~50 linhas, Composition Root)
                    ================================================

ntb_ia_{specialty}_motor.py
  1. dbutils.widgets (ambiente, catalogo, data, specialty_id)
  2. config = yaml.safe_load("config.yaml")
  3. df = data_manage.load(config["data"])
  4. result = nlp_engine.process(df, config["nlp"])
  5. monitoring.emit(result, config["monitoring"])
  6. data_manage.save(result, config["data"])
...
| ----------------------- | --------------------------- | -------------------------------------------------------------- |
| Logica NLP              | Dentro do notebook          | 3 libs independentes (wheel)                                   |
| Configuracao clinica    | Hardcoded                   | YAML versionado (Git)                                          |
| Compartilhamento        | Copy-paste                  | `import nlp_engine`, `import data_manage`, `import monitoring` |
...
| Adicionar especialidade | Copiar repo, adaptar        | Copiar _template, ajustar YAML                                 |
