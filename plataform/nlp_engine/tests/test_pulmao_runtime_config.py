...
def test_pulmao_yaml_merge_with_shared_organs_has_expected_keys() -> None:
    specialty = _load_yaml(_PULMAO_CFG)
    shared = _load_yaml(_SHARED_ORGANS)
    merged = merge_with_shared_organs(shared, specialty)

    assert merged["specialty_id"] == "pulmao"
    assert merged["config_version"].startswith("0.1.")
    assert merged["nlp"]["target_organs"] == ["pulmao"]
    assert "pulmao" in merged["nlp"]["organs"]
    assert "torax" in merged["nlp"]["all_organs"]
    assert "pulmao" in merged["nlp"]["header_aliases"]


def test_pulmao_yaml_invalid_missing_specialty_id() -> None:
    specialty = _load_yaml(_PULMAO_CFG)
